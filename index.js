// Завдання: 1
/**
 * Функція `customSet` створює множину з масиву чисел та рядків,
 * але видаляє всі елементи, які є числами.
...
function customSet(arr) {
  ...
  // Перебираємо множину за допомогою 'for of'. Цикл 'for of' дозволяє
  //пройтись по всіх елементах колекції.
  // Для кожного елемента перевіряємо, чи є цей елемент числом за
...
function clearSet(set) {
  // Використання властивості size для перевірки розміру множини,
  //чи більше вона нуля
  // Якщо так використання методу clear для очищення множини та повертаємо
  //Множину очищено.
...
// Завдання: 3
/**
 * Функція `addElements` додає елементи до множини з масиву,
 * якщо вони ще не присутні у множині.
...
function addElements(mySet, arr) {
  // Перебираємо масив та додаємо кожний елемент до множини за
  // допомогою методу add
  // Метод add додає елемент до множини лише якщо він ще не присутній
  //у множині
...
function filterAndAdd(mySet, arr) {
  // Перебираємо множину за допомогою 'for of'. Всередині циклу перевіряємо,
  // чи є поточний елемент числом.
...
/**
 * Функція `checkValueAndType` перевіряє, чи містить множина певне значення
 * та виводить його тип.
 *
...
function checkValueAndType(mySet, value) {
  ...
  // Якщо значення знайдено, повертаємо рядок Множина
  //має значення "${value}" типу "${typeof value}".
  // Якщо значення не знайдено, повертаємо рядок Множина не
  // має значення "${value}".
  if (mySet.has(value)) {
...
/**
 * Функція `setToArray` конвертує множину в масив, видаляє з масиву всі
 * числові елементи
...
function setToArray(mySet) {
  ...
  // Використовуємо метод filter для створення нового масиву, що містить
  //лише рядкові елементи.
  // Використовуємо метод sort для сортування рядкових елементів в алфавітному
  //порядку.
...
/**
 * Функція `removeDuplicatesInPlace` видаляє дублікати 
 * з масиву без створення нового масиву.
...
function areDisjoint(set1, set2) {
  ...
  // Якщо знайдено спільний елемент з другою множиною,використовуємо 
  //метод has, повертаємо false
...
/**
 * Функція `iterateSet` виконує ітерацію по множині та виводить ключі,
 *  значення та записи кожного елемента.
